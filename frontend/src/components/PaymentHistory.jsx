...
const PaymentHistory = ({ userId }) => {
    ...
    useEffect(() => {
        const fetchPayments = async () => {
            try {
                ...
                setError('Error fetching payment history: ' + error.response?.data?.detail || error.message);
            }
        };
        fetchPayments();
    }, [userId]);
    ...
    const chartData = {
        labels: payments.map(payment => payment.created_at), // Use created_at for labels
        datasets: [
            {
                label: 'Amount',
    ...
    return (
        <div className="max-w-6xl mx-auto p-6 bg-gray-100 min-h-screen">
            <h2 className="text-3xl font-bold text-center mb-8">Your Payment History</h2>
                    ...
                    <tbody className="text-gray-600 text-sm font-light">
                        {payments.map(payment => (
                            <tr key={payment.id} className="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td className="py-3 px-6 text-left">{new Date(payment.created_at).toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <div className="bg-white p-6 shadow-md rounded-lg">
                <h3 className="text-xl font-semibold mb-4 text-center">Payment Amounts</h3>
                <div className="h-64">
                    <Bar data={chartData} />
                </div>
            </div>
